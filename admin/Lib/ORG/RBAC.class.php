...
/**
 +------------------------------------------------------------------------------
 * 基于角色的数据库方式验证类，该RBAC修改于THINKPHP的RBAC类
 +------------------------------------------------------------------------------
 */
// 配置文件增加设置
// USER_AUTH_ON 是否需要认证
...
class RBAC extends Think
{
    ...
    {    	
//    	echo MODULE_NAME." ".ACTION_NAME;exit;
        //检查是否需要认证
        if(RBAC::checkAccess()) {
        	
        	//add by chenfq 2010-06-08 begin 无法退出问题
            //检查认证识别号
            if(!$_SESSION[C('USER_AUTH_KEY')]) {
                // 禁止游客访问跳转到认证网关
                redirect(PHP_FILE.C('USER_AUTH_GATEWAY'));
            }        
            //add by chenfq 2010-06-08 end
    ...
    static public function getAccessList($authId)
    {
        ...
        $db     =   Db::getInstance();
        $auth_type = C("AUTH_TYPE");
        $table = array('role'=>C("DB_PREFIX").C('RBAC_ROLE_TABLE'),'user'=>C("DB_PREFIX").C('RBAC_USER_TABLE'),'access'=>C("DB_PREFIX").C('RBAC_ACCESS_TABLE'),'node'=>C("DB_PREFIX").C('RBAC_NODE_TABLE'),'nav'=>C("DB_PREFIX").C('RBAC_NAV_TABLE'));
        $sql    =   "select node.auth_type,node.id,node.action,node.action_name,node.module,node.module_name,nav.id,nav.name from ".
                    $table['role']." as role,".
                    $table['user']." as user,".
        ...
        foreach($apps as $key=>$app) {
            ...
            {
            	$access[strtoupper($module_name)][strtoupper($action_name)]   =  true;     
            }
        	if($app['auth_type']==1) //模块授权
            {
            	$access[strtoupper($module_name)][strtoupper($auth_type[$app['auth_type']])]   =  true;     
            }
       		if($app['auth_type']==2) //操作授权
            {
            	$access[strtoupper($auth_type[$app['auth_type']])][strtoupper($action_name)]   =  true;     
            }
