...
include_once (VENDOR_PATH . 'user_login/2345/Tuan2345Auth.php');
define("APP_KEY", a_fanweC('2345_KEY'));       // input your key
define("APP_SECRET", a_fanweC('2345_SECRET')); //input you secret
if ($_REQUEST['oauth_2345'] == '1') {
	
    $callback = 'http://' . $_SERVER['HTTP_HOST'] . __ROOT__ . "/index.php?m=User&a=login_2345&goods_id=" . intval($_REQUEST ['id']);
...
//var_dump($token);
    ...
    $is_2345= tuan2345_info();
    $u_id = $_SESSION['t2345id'];
    $user_data = $GLOBALS['db']->getRow("select * from " . DB_PREFIX . "user where 2345_id = '" . $_SESSION['t2345id'] . "' and 2345_id <> 0");
    $names = $GLOBALS['db']->getRow("select * from " . DB_PREFIX . "user where user_name = '" . trim($_SESSION['qname']) . "' ");
    $has_name = empty($names) ? 0 : 1;
    if (!$user_data && !$has_name && !empty($_SESSION['qname'])) {
        $data ['user_name'] = trim($_SESSION ['qname']);
        $data ['user_pwd'] = trim($_SESSION ['qmail']);
        $data ['email'] = trim($_SESSION ['qmail']);
        $data ['2345_id'] = trim($_SESSION ['t2345id']);
        $data ['status'] = 1;
    ...
    if ($user_data) {
        ...
        $GLOBALS ['tpl']->assign('user_api_field_name', '2345_id');
        $GLOBALS ['tpl']->assign('user_api_field_value', intval($_SESSION['t2345id']));
...
function tuan2345_info() {
    ...
    if (!empty($_REQUEST['oauth_token']) && !empty($_REQUEST['oauth_verifier'])) {
        ...
        if (!empty($errorno)) {
            ...
            if (empty($userinfo_arr[2])) {
                $accessToken["qmail"] = $userinfo_arr[2];
            } elseif (empty($userinfo_arr[1])) {
                $accessToken["qname"] = $userinfo_arr[1];
            }
           
            $_SESSION['t2345id'] = $accessToken["user_id"];
            $_SESSION['qname'] = $accessToken["qname"];
            $_SESSION['qmail'] = $accessToken["qmail"];
            return true;
        }
    } elseif (!empty($_REQUEST['user_id']) && (!empty($_REQUEST['qname']) || !empty($_REQUEST['qmail'])) && !empty($_REQUEST['sign']) && ($_REQUEST['from'] == 'tuan2345')) {
        ...
        $_SESSION['t2345id'] = $_REQUEST["user_id"];
