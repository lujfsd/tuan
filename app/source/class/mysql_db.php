...
class mysql_db
{
    ...
    var $cache_data_dir = 'app/Runtime/caches/';
    var $root_path      = '';
...
/* 仿真 Adodb 函数 */
    ...
    function getSqlCacheData($sql, $cached = '')
    {
        ...
        //chenfq 加入缓存文件分目录 2011-10-03
        $hash_dir = $this->root_path . $this->cache_data_dir . substr(md5($sql), 0, 1)."/".substr(md5($sql), 1, 1)."/".substr(md5($sql), 2, 1)."/";
        if (!is_dir($hash_dir))
        {
        	mysql_db::mkdirs($hash_dir);
        	if(function_exists("chmod"))
        		chmod($hash_dir, 0777);
        }
                
        $result['filename'] = $hash_dir . 'sqlcache_' . abs(crc32($this->dbhash . $sql)) . '_' . md5($this->dbhash . $sql) . '.php';
    ...
    function mkdirs($dir){
    	 if(!is_dir($dir))  
		 {  
			 if(!mysql_db::mkdirs(dirname($dir)))
			 {
			 	return false; 
			 }  
			 if(!mkdir($dir,0777))
