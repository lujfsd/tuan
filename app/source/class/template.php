    ...
    function display($filename, $cache_id = '')
    {
     ...
     /*******************************替换又拍云图片地址*********************************************/
        ...
        if($upyun_conf['status']){
            $string = $out;
            $parent = array(CND_URL."/upyun/Public",CND_URL."//upyun/Public");
            $replace = array($upyun_conf['url']. "/Public",$upyun_conf['url']. "/Public");
			
			$out = str_replace($parent,$replace, $string);
        }
        else{
        	$string = $out;
            $parent = array(CND_URL."/upyun/Public",CND_URL."//upyun/Public");
            $replace = array(CND_URL. "/Public",CND_URL. "/Public");
			$out = str_replace($parent,$replace, $string);
        }
        /*******************************END又拍云图片地址*********************************************/
...
function callback_fetch_str($matches)
{
    ...
    function fetch_str($source)
    {
       ...
       // return preg_replace_callback("/{([^\}\{\n]*)}/",function ($matches){return $this->select($matches[1]);},$source);
        return preg_replace_callback("/{([^\}\{\n]*)}/",array(&$this,'callback_fetch_str'),$source);
    }
...
function callback_get_val($matches)
{
...
//            $val = preg_replace("/\[([^\[\]]*)\]/eis", "'.'.str_replace('$','\$','\\1')", $val);
            ...
            $val = preg_replace_callback("/\[([^\[\]]*)\]/is", array(&$this,'callback_get_val'),$val);
        }
