...
{//类定义开始
    ...
    public function __construct($options='')
    {
        ...
        $this->connected = is_dir($this->options['temp']); //修改 by hc 去除is_writeable判断。有时不准确导至cache connect失败 
    ...
    private function filename($name)
    {
        ...
        if(C('DATA_CACHE_SUBDIR')) {
            ...
            if(preg_match ("/CACHE_ORDER_LIST_$user_id_/i", $oname)
            ||preg_match ("/CACHE_BELOW_ORDER_LIST_$user_id_/i", $oname)
            ||preg_match ("/CACHE_ECV_LIST_$user_id_/i", $oname)
            ||preg_match ("/CACHE_GROUP_BOND_LIST_$user_id_/i", $oname)
            )
            {
            	$dir = "user_order_cache_".$user_id.'/';  
            }
            else
            {            
