    ...
    public function select($options=array()) {
    	...
    	{    		
    		if(C('DB_FIELDS_CACHE'))
    		{
	    		...
	    		$module_name = parse_name(str_replace(C("DB_PREFIX"),"",$tb_name),1);    		
	    		$tb_field = F('_fields/'.$module_name);	 	    		
	    		unset($tb_field['_autoinc']);
	    		unset($tb_field['_pk']);
				foreach($tb_field as $k=>$v)
				{
					$tb_field[$k] = "`".$v."`";
				}
	    		if($tb_field)
	    		{  		
    				$options['field'] = implode(",",$tb_field);
	    		}
    		}
    	}
