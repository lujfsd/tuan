...
function U($url,$params=array(),$redirect=false,$suffix=true) {
    ...
    if((APP_NAME=='app'&&$module!='z'&&!isset($query['cityname']))||APP_NAME=='mobile')
    {
    	//全局插入cityName 
        if(fanweC("CITYNAME_URL")==1)
        {
        	if(isset($_REQUEST['cityname']))
        	{
        		$citypy = $_REQUEST['cityname'];
        	}
        	else 
        	{
        		$citypy = $_SESSION['cityName'];
        	}

	        $cityparam = array("cityname"=>$citypy);
	        $params = array_merge($cityparam,$params);
        }
        
	    if(C("SESSION_URL")==1)
	    {	    	
	    	$session_param = array("s"=>SESSION_ID);
	        $params = array_merge($session_param,$params);	        
	    }
    }
    if((C('URL_DISPATCH_ON') && C('URL_MODEL')>0)||(C('URL_DISPATCH_ON') && fanweC('URL_MODEL')>0&&APP_NAME=='app')) {
...
{
	if ($name == 'DEFAULT_LANG'){ //chenfq 2011-04-01 添加
		//return 'zh-cn';//
		return fanweC('DEFAULT_LANG');
	}
...
function strip_whitespace($content) {
	if (!function_exists('token_get_all')) {
		if (function_exists('php_strip_whitespace')) {
			return php_strip_whitespace($content); 
		}else{
			echo 'token_get_all on exists';exit;
		}				
	}	
    $stripStr = '';
    //分析php源码
