...
// modified by zen-cart.cn
  ...
  function Execute($zf_sql, $zf_limit = false, $zf_cache = false, $zf_cachetime=0) {
    ...
    if (strtoupper(substr($zf_sql,0,6))=='SELECT' || strtoupper(substr($zf_sql,0,4))=='SHOW'){
    ...
    function getOne($sql, $field_name = '')
    {
        $res = $this->Execute($sql);
        if (empty($field_name)){
        	$fields = array_keys( $res->fields);
        	return $res->fields[$fields[0]];
        }else{
  ...
  function ExecuteRandomMulti($zf_sql, $zf_limit = 0, $zf_cache = false, $zf_cachetime=0) {
    ...
    $obj = new queryFactoryResult;
    $obj->result = array();
    if (!$this->db_connected) $this->set_error('0', 'Error - Could not connect to Database');
    $zp_db_resource = @mysql_query($zf_sql, $this->link);
