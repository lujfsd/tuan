...
class SgListView
{
    ...
    function CountRecord()
    {
        ...
        if(count($this->searchArr) > 0)
        {
            if(!empty($this->searchArr['nativeplace']))
            {
                ...
                {
                    $naddQuery .= " AND arc.nativeplace >= '{$this->searchArr['nativeplace']}' AND arc.nativeplace < '".($this->searchArr['nativeplace']+499)."'";
                }
                else
                {
            ...
            if(!empty($this->searchArr['infotype']))
            {
                ...
                {
                    $naddQuery .= " AND arc.infotype >= '{$this->searchArr['infotype']}' AND arc.infotype < '".($this->searchArr['infotype']+499)."'";
                }
                else
                {
	     ...
	     //修改start
	     if(!empty($this->searchArr['itemtype']))
		 {
			 if($this->searchArr['itemtype'] % 500 ==0 )
			 {
				 $naddQuery .= " And arc.itemtype >= '{$this->searchArr['itemtype']}' And arc.itemtype < '".($this->searchArr['itemtype']+499)."'";
				 }
				 else
				 {
					 $naddQuery .= "And arc.itemtype = '{$this->searchArr['itemtype']}'";
				 }
		     }
		 if(!empty($this->searchArr['investment']))
		 {
			 if($this->searchArr['investment'] % 500 ==0 )
			 {
				 $naddQuery .= " And arc.investment >= '{$this->searchArr['investment']}' And arc.investment < '".($this->searchArr['investment']+499)."'";
				 }
				 else
				 {
					 $naddQuery .= "And arc.investment = '{$this->searchArr['investment']}'";
				 }
		     }	 
		...
		//排序修改start
	    //echo 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];//输出当前网址
			if(!empty($_GET['odrby']))
            {
				switch ($_GET['odrby'])
				{
				case 1:
				  //$orderQuery .=" ORDER BY click desc";
				  $orderQuery = " ORDER BY arc.click desc";
				  break;  
				case 2:
				  $orderQuery = " ORDER BY arc.joincost desc";
				  break;
				case 3:
				  $orderQuery = " ORDER BY arc.senddate desc";
				  break;
				default:
    ...
    function GetArcList($limitstart=0,$row=10,$col=1,$titlelen=30,$listtype="all",$orderby="default",$innertext="",$tablewidth="100",$ismake=1,$orderWay='desc')
    {
		...
		else if($orderby=='joincost')
        {
            $ordersql = " ORDER BY arc.joincost $orderWay";
        }
        else
        {
            $ordersql=" ORDER BY arc.aid $orderWay";
        ...
        //{
            ...
            tp.ispart,tp.moresite,tp.siteurl,tp.sitepath,arc.aid,arc.aid AS id,arc.typeid,arc.joincost,arc.summary,arc.stitle,
            $addField
        ...
        for($i=0;$i<$row;$i++)
        {
            ...
            for($j=0;$j<$col;$j++)
            {
                if($row = $this->dsql->GetArray("al"))
                {
					...
					$row['joincosts'] = $row['joincost'];
					$row['summarys'] = $row['summary'];
					$row['stitles'] = $row['stitle'];
